...
#ตัวอย่าง Message Type "Text"
    if($message == "สวัสดี"){
        ...
        replyMsg($arrayHeader,$arrayPostData);
			...
			$url = "http://injaimetech.com/bon_sql.php?id=".$id;
	...
	else if(strtolower($message) == "orp"){
        $url = "https://injaimetech.com/erdiboy/getdatanow.php?target=orp";
		$json = file_get_contents($url);
		$obj = json_decode($json);

		$arrayPostData['replyToken'] = $arrayJson['events'][0]['replyToken'];
		$arrayPostData['messages'][0]['type'] = "text";
		$arrayPostData['messages'][0]['text'] = "ค่า ORP ในขณะนี้อยู่ที่ ".$obj[1] ." mV";
		replyMsg($arrayHeader,$arrayPostData);
	}
	else if(strtolower($message) == "ph"){
		$url = "https://injaimetech.com/erdiboy/getdatanow.php?target=ph";
		$json = file_get_contents($url);
		$obj = json_decode($json);
		
		$arrayPostData['replyToken'] = $arrayJson['events'][0]['replyToken'];
        $arrayPostData['messages'][0]['type'] = "text";
        $arrayPostData['messages'][0]['text'] = "ค่าความเป็นกรด-เบส ในขณะนี้อยู่ที่ ".$obj[1];
        replyMsg($arrayHeader,$arrayPostData);
	}
	else if(strtolower($message) == "temp"){
		$url = "https://injaimetech.com/erdiboy/getdatanow.php?target=temp";
		$json = file_get_contents($url);
		$obj = json_decode($json);
		
		$arrayPostData['replyToken'] = $arrayJson['events'][0]['replyToken'];
        $arrayPostData['messages'][0]['type'] = "text";
        $arrayPostData['messages'][0]['text'] = "ค่าอุณหภูมิในขณะนี้อยู่ที่ ".$obj[1] ." °C";
        replyMsg($arrayHeader,$arrayPostData);
	}
	else{
	    $arrayPostData['replyToken'] = $arrayJson['events'][0]['replyToken'];
